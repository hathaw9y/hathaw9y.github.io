...
FPGA 관점에서 WS PE는 weight 저장 register를 하나 더 가진 MAC pipeline으로 볼 수 있다.
weight를 PE 내부 register에 오래 잡아두기 때문에 같은 weight를 여러 activation에 재사용할 수 있지만, compute 전에 weight를 올바른 PE 위치까지 밀어 넣는 load phase가 필요하다.
즉 WS 설계의 어려움은 곱셈 자체보다 weight load timing과 partial sum valid timing을 맞추는 데 있다.
...
따라서 WS 배열의 출력은 OS처럼 `ROWS x COLS` tile 전체가 한 번에 나오는 형태가 아니다.
한 compute wave가 끝날 때 배열 아래쪽에서 `COLS`개 결과 vector가 나오고, 외부 FSM이 이를 BRAM에 저장하거나 이전 partial sum과 다시 더한다.
이 차이 때문에 WS에서는 Engine 단계에서 Accumulator BRAM read path가 추가된다.
